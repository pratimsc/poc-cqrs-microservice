...
object TechnicalRulesFactory {
  def isValidPaymentAmount(amount: Int) = new Rule {
    override def validate(): Result = if (amount > 0)
      Success
    else
      Failure(Seq(Error(uUID = UUID.randomUUID(),
        code = "PMT001",
        title = "Invalid payment amount",
        detail = s"Amount ${amount} must be greater than 0 (Zero) ")))
  }
  ...
  def isValidCurrency(currency: String) = new Rule {
    ...
    override def validate(): Result = if (CURRENCIES.contains(currency))
      ...
      Failure(Seq(Error(uUID = UUID.randomUUID(),
        ...
        detail = s"Currency ${currency} is not supported. Supported currencies are [${CURRENCIES.mkString(",")}] ")))
  }
